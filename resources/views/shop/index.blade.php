...
@section('content')
    <div class="row">
        <div class="col s12 m3 l3">
            ...
            <ul class="cats">
                @foreach ($cats as $cat)
                    <li><a href="{{ route('shop.cat', $cat->slug) }}" class="valign-wrapper"><img src="{{$cat->img}}"> <span class="valign">{{$cat->title}}</span></a></li>
                @endforeach
            </ul>
        </div>
        <div class="col s12 m9 l9">
            ...
            @if (Auth::user())
                @if (Auth::user()->hasRole('Freelancer'))
                    <a href="{{ route('shop.create') }}" class="right btn waves-effect prj_create_proect blue"><i class="material-icons">add</i> <span>Добавить товар</span></a>
                @endif
            @endif
            <div class="row">
